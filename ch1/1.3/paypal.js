...
class Paypal extends Client {
  ...
  checkTxNonce(tx) {
    ...
    } else if(tx.contents.nonce === this.state[tx.contents.from].nonce) { // if the transaction nonce is the same as the nonce Paypal has for that account
      // return true to signal that it is ok to proceed to the nex operation
      return true;
    } else if(tx.contents.nonce < this.state[tx.contents.from].nonce) { // if the transaction nonce is less than the nonce Paypal has for that account
  ...
  checkTxType(tx) {
    ...
    if (tx.contents.type === 'cancel') {
      ...
      this.txHistory.forEach(processedTx => {
        // if the nonce matches delete the transaction and reverse the old transaction
        if(nonce === processedTx.contents.nonce) {
          // make sure that the user who is cancelling the transaction is the same user who signed the transaction to be cancelled
          if(tx.contents.from === processedTx.contents.from) {
            // take money back from the receiver
            this.state[processedTx.contents.to].balance -= processedTx.contents.amount;
            // give the sender back their money
            this.state[processedTx.contents.from].balance += processedTx.contents.amount;
            ...
            this.state[processedTx.contents.from].balance -= 1;
  ...
  processPendingTx() {
    ...
    this.pendingTx.forEach(tx => {
      ...
      const stateNonce = this.state[sender].nonce;
